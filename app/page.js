...
import { RefreshCw, Activity, ChevronRight, ChevronLeft, Settings, Hammer } from 'lucide-react';
...
export default function Home() {
  ...
  const materials = {
    rubber: { restitution: 0.95, friction: 0.1, density: 0.01, color: '#22d3ee', name: 'Rubber' }, // Cyan
    wood: { restitution: 0.4, friction: 0.4, density: 0.04, color: '#d97706', name: 'Wood' },     // Amber
    metal: { restitution: 0.1, friction: 0.2, density: 0.1, color: '#94a3b8', name: 'Metal' },    // Slate
    ice: { restitution: 0.2, friction: 0.001, density: 0.02, color: '#bae6fd', name: 'Ice' }      // Sky Blue
  };

  useEffect(() => {
    const { Engine, Render, Runner, Bodies, World, Mouse, MouseConstraint } = Matter;

    // Setup Engine with Tunneling Fixes
  ...
  // 🚀 NEW: Custom Object Spawner
  const spawnCustomObject = () => {
    if (!engineRef.current) return;
    const { Bodies, World } = Matter;
    
    // Drop point based on lesson (Lesson 2 drops on the high side of the ramp)
  ...
  return (
    <main className="relative min-h-screen bg-slate-950 text-slate-100 overflow-hidden font-sans">
      ...
      <div className="absolute top-0 left-0 w-full p-6 z-30 pointer-events-none flex justify-between items-start">
        ...
        <div className="bg-slate-900/90 backdrop-blur-md border border-slate-800 p-6 rounded-2xl shadow-2xl pointer-events-auto max-w-md w-full">
                  ...
                  <div className="flex gap-2">
                    {['circle', 'square', 'triangle'].map(shape => (
                      <button 
                        ...
                        className={`capitalize flex-1 py-1.5 text-xs rounded-md border ${customShape === shape ? 'bg-cyan-900/50 border-cyan-500 text-cyan-300' : 'bg-slate-800 border-slate-700 text-slate-400'}`}
                      >
                        {shape}
                      </button>
                    ))}
